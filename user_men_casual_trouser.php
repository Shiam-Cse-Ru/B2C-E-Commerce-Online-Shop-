            ...
            <div id="content">
                <div class="hero-unit-table"> 
                    ...
                    <div id="body">
                        ...
                        <ul class="thumbnails">
                            <?php
                            $query = mysqli_query($dbcon,"select * from tb_products WHERE category = 'Men_Casual_Trouser'") or die(mysqli_error());
                            while ($row = mysqli_fetch_array($query)) {
                                $id = $row['productID'];
								$qty = $row['quantity'];
								
									$query1 = mysqli_query($dbcon,"SELECT *,SUM(qty) as qty FROM order_details WHERE productID = '$id' AND status = 'Delivered'");
                                ...
                                <li class="span3">
                                    ...
                                    if (isset($_POST['order'])) {
                                        ...
                                        mysqli_query($dbcon,"insert into order_details (memberID,qty,productID,price,total,status) values('$member_id','$quantity','$product_id','$price','$total','Pending')") or die(mysql_query);
                                     /*    header('location:user_wines.php'); */
										?>
												<script>
																window.location = 'user_men_casual_trouser.php';				
																</script>
										<?php
                                    }
                                    ?>
